...
#include <utils/Log.h>
#include "pn544_firmware.h"

const unsigned char *nxp_nfc_full_version = pn544_nxp_nfc_full_version;
const unsigned char *nxp_nfc_fw = pn544_nxp_nfc_fw;

/* The stock DSO contains firmware arrays, not host code needed by NFC.
 * Extract them at build time so no legacy ELF text relocations are loaded.
 */
int dlopen_firmware(void) {
    return 0;
}
